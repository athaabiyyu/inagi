<?php
/**
 * The template for displaying 404 page (not found) for V4.
 *
...
<section class="d-flex align-items-center min-vh-100 py-5 bg-secondary">
	<div class="container d-flex justify-content-center text-center">
		<div class="col-lg-8 col-md-10 col-12 px-0">
			<div class="ratio ratio-16x9 mb-lg-5 mb-4">
				<?php
					$image_option = wp_get_attachment_image(
						...
						array(
							'class' => 'w-100',
							'alt'   => esc_html_x( '404', 'front-end', 'finder' ),
						)
					);
					if ( '' !== $image_option ) :
						...
						?>
				<lottie-player src="<?php echo esc_attr( FINDER_THEME_URI . 'assets/json/animation-city-guide-404.json' ); ?>" background="transparent" speed="1.5" loop autoplay></lottie-player>
				<?php endif; ?>
			</div>
			<?php if ( get_theme_mod( 'finder_404_title' ) !== '' ) : ?>
				<h1 class="404_title h3 pt-lg-4"><?php echo esc_html( get_theme_mod( 'finder_404_title', esc_html_x( 'Page Not Found.', 'front-end', 'finder' ) ) ); ?></h1>
			<?php endif; ?>
			<?php if ( get_theme_mod( 'finder_404_description' ) !== '' ) : ?>
				<p class="404_desc lead mb-5 pb-lg-2"><?php echo esc_html( get_theme_mod( 'finder_404_description', esc_html_x( 'Sorry, we can’t find the page you are looking for. We suggest you go to homepage while we are fixing the problem.', 'front-end', 'finder' ) ) ); ?></p>
			<?php endif; ?>
			<a
				<?php
					finder_render_attr(
