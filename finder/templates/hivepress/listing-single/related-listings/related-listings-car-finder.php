	...
	<div class="card card-light card-hover h-100">
		<div class="card-img-top card-img-hover">
							...
							foreach ( $listing->_get_fields( 'view_block_primary' ) as $key => $field ) {
								...
								if ( ! is_null( $field->get_value() ) ) {
									...
									if ( 'badge' === $attribute_view_style ) {
										?>
										<span class="d-table badge bg-primary mb-1"><?php echo wp_kses_post( finder_hivepress_get_attribute_display( $display, 'archive_listing_icon_classes', $attribute_id ) ); ?></span>
										<?php
			...
			<?php if ( $listing_image_url ) : ?>
				<div class="aspect-ratio aspect-w-219 aspect-h-112">
					...
					foreach ( $listing->_get_fields( 'view_block_primary' ) as $key => $field ) {
						...
						if ( ! is_null( $field->get_value() ) ) {
							...
							if ( 'price' === $attribute_view_style ) {
								...
								?>
								<div class="text-light opacity-70 mb-1"><?php echo wp_kses_post( finder_hivepress_get_attribute_display( $display, 'archive_listing_icon_classes', $attribute_id ) ); ?></div>
								<?php
							}
						}
					}
						...
						foreach ( $listing->_get_fields( 'view_block_secondary' ) as $key => $field ) {
							...
							if ( ! is_null( $field->get_value() ) ) {
								...
								<div class="<?php echo esc_attr( $col_class ); ?>">
									...
									$col_class = ( $def_attr === $i ) ? 'col' : 'col me-sm-1';
									?>
									<div class="<?php echo esc_attr( $col_class ); ?>">
										<div class="bg-dark rounded text-center w-100 h-100 p-2">
											<span class="fs-xs text-light"><?php echo wp_kses_post( finder_hivepress_get_attribute_display( $display, 'archive_listing_icon_classes', $attribute_id ) ); ?></span>
