<?php
/**
 * Listing real estate content template.
 *
 * @package Finder
 */
...
<div class="card shadow-sm card-hover border-0 h-100">
				...
				<?php foreach ( $listing_images as $image_url ) : ?>
					<div>
						<div class="aspect-ratio aspect-w-219 aspect-h-100">
							<img class="w-full h-full object-center object-cover" src="<?php echo esc_url( $image_url ); ?>" alt="<?php echo esc_attr( $listing->get_title() ); ?>" loading="lazy">
						</div>
					</div>
				<?php endforeach; ?>
			<?php else : ?>
	...
	<div class="card-body position-relative pb-3">
		<?php if ( $listing->get_categories__id() ) : ?>
			<h4 class="finder-hp-listing-category mb-1 fs-xs fw-normal text-uppercase text-primary">
		...
		?>
		<?php if ( $listing->_get_fields( 'view_block_primary' ) && finder_hivepress_default_attribute_count( $listing, 'view_block_primary' ) ) : ?>
	...
	<?php if ( $listing->_get_fields( 'view_block_secondary' ) && ! empty( finder_hivepress_default_attribute_count( $listing, 'view_block_secondary' ) ) ) : ?>
		<div class="card-footer d-flex flex-wrap align-items-center justify-content-center mx-3 pt-3 text-nowrap hp-listing__attributes hp-listing__attributes--secondary">
