	...
	if ( 'city-guide' === $account_style ) {
		...
		?>
			<div class="card card-body p-4 p-md-5 shadow-sm">
				...
				<div class="d-flex flex-md-row flex-column align-items-md-center justify-content-md-between mb-4 pt-2">
					<h1 class="<?php echo esc_attr( $header_class ); ?>"><?php echo esc_html__( 'Listings', 'finder' ); ?></h1>
				</div>
			...
			<?php if ( class_exists( 'HivePress\Components\Import') ){?>
					<button type="button" class="hp-listing__action hp-listing__action--import button button--secondary btn btn-primary btn-sm" data-component="link" data-url="<?php echo esc_url( hivepress()->router->get_url( 'listing_import_page' ) ); ?>"><span><?php echo esc_html( hivepress()->translator->get_string( 'import_listings' ) ); ?></span></button><?php
				}
			?>
			<?php
		...
		<div class="row">
			...
			<div class="col-lg-8 col-md-7 mb-5">
				...
				?>
				<?php
				if ( class_exists( 'HivePress\Components\Import') ){?>
					<button type="button" class="hp-listing__action hp-listing__action--import button button--secondary btn btn-primary btn-sm" data-component="link" data-url="<?php echo esc_url( hivepress()->router->get_url( 'listing_import_page' ) ); ?>"><span><?php echo esc_html( hivepress()->translator->get_string( 'import_listings' ) ); ?></span></button><?php
				}
				?>
			</div>
