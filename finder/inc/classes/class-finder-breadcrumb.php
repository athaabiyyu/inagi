...
class Finder_Breadcrumb {
	...
	protected function add_crumbs_product_tag() {
		...
		$this->add_crumb( sprintf( __( 'Products tagged &ldquo;%s&rdquo;', 'finder' ), $current_term->name ), get_term_link( $current_term, 'product_tag' ) );
	}
	...
	protected function add_crumbs_tag() {
		...
		$this->add_crumb( sprintf( __( 'Posts tagged &ldquo;%s&rdquo;', 'finder' ), single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
	}
	...
	protected function search_trail() {
		if ( is_search() ) {
			...
			$this->add_crumb( sprintf( __( 'Search results for &ldquo;%s&rdquo;', 'finder' ), get_search_query() ), remove_query_arg( 'paged' ) );
		}
	}
