...
if ( ! class_exists( 'WP_Bootstrap_Navwalker' ) ) :
	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = null ) {
			...
			if ( finder_is_header_navbar_dark() && 'primary' === $args->theme_location ) {
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			foreach ( $classes as $key => $class ) {
				if ( preg_match( '/^border-(\S*)?|^border(\s?)$/i', $class ) ) {
					// Anchor Border Class.
					$anchor_border_classes[] = $class;
					unset( $classes[ $key ] );
				}
			}

			$anchor_border_classes = join( ' ', $anchor_border_classes );
			...
			if ( isset( $args->item_class ) && is_array( $args->item_class ) ) {
				$classes[] = join( ' ', $args->item_class );
			}
			...
			if ( $anchor_border_classes ) {
				$anchor_border_classes = $anchor_border_classes . ' align-items-center py-1 pe-lg-4';

				if ( finder_is_header_navbar_dark() ) {
					$anchor_border_classes .= ' border-light';
				}

				$atts['class'] .= ' ' . $anchor_border_classes;
			}
			...
			if ( ! empty( $icon_class_string ) ) {
				if ( 'primary' === $args->theme_location && $depth > 0 ) {
					$icon_class_string .= ' fs-base opacity-50';
				}
				if ( 'primary' === $args->theme_location || 'offcanvas_menu' === $args->theme_location ) {
					$icon_class_string .= ' me-2';
				}
		...
		public static function fallback( $args ) {
			...
			$fallback_output .= '<li class="nav-item"><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" class="nav-link" title="' . esc_attr__( 'Add a menu', 'finder' ) . '">' . esc_html__( 'Add a menu', 'finder' ) . '</a></li>';
			$fallback_output .= '</ul>';
			...
			if ( array_key_exists( 'echo', $args ) && $args['echo'] ) {
				echo apply_filters( 'finder_bs_navwalker_fallback_output', $fallback_output ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			} else {
		...
		private function separate_linkmods_and_icons_from_classes( $classes, &$linkmod_classes, &$icon_classes, $depth ) {
			...
			foreach ( $classes as $key => $class ) {
				...
				if ( preg_match( '/^disabled|^visually-hidden/i', $class ) ) {
					...
					$icon_classes[] = $class;
					unset( $classes[ $key ] );
				} elseif ( preg_match( '/^fi-(\S*)?|^fi(\s?)$/i', $class ) ) {
					// Findericons.
					$icon_classes[] = $class;
					unset( $classes[ $key ] );
				}
