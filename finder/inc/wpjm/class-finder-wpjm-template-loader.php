<?php
/**
 * Finder Job Manager Template Loader Class
 *
 * @package Finder
 */
 ...
 */
class Finder_WPJM_Template_Loader {
	...
	public static function init() {
		self::$jobs_page_id = finder_wpjm_get_page_id( 'jobs' );
	...
	public static function template_loader( $template ) {
		...
		if ( $default_file ) {
			/**
			 * Filter hook to choose which files to find before finder does it's own logic.
			 *
			 * @since 3.0.0
			 * @var array
			 */
	...
	private static function get_template_loader_default_file() {
		...
		} elseif ( current_theme_supports( 'job-manager-archive' ) && ( is_post_type_archive( 'job_listing' ) || is_page( self::$jobs_page_id ) || finder_is_job_listing_taxonomy() ) ) {
	...
	private static function get_template_loader_files( $default_file ) {
		$templates = apply_filters( 'finder_wpjm_template_loader_files', array(), $default_file );
		...
		if ( finder_is_job_listing_taxonomy() ) {
			$object      = get_queried_object();
...
add_action( 'init', array( 'Finder_WPJM_Template_Loader', 'init' ) );
